...
export function generateSvgUri(uglyInput) {
    ...
    //Some params for the background
    const someStrings = [
        "&#128220;",
        "&#128214;",
        "&#128209;",
    ...
    let prettyInputOverride = [
        someStrings[0], 
        Math.random() < 0.75 ? someStrings[0] : someStrings[Math.floor(Math.random()*someStrings.length)],
        Math.random() < 0.75 ? someStrings[0] : someStrings[Math.floor(Math.random()*someStrings.length)],
        Math.random() < 0.75 ? someStrings[0] : someStrings[Math.floor(Math.random()*someStrings.length)],
        someStrings[0]
    ].join('');
    ...
    let imgCode = '<svg viewBox="0 0 142 142" version="1.1" xmlns="http://www.w3.org/2000/svg">'
        ...
        + '<path d="' + curveFromPoints(points, 0.65, thetaParam) + '" fill="url(#Gradient)" opacity="0.99" />'
        + '</svg>';
...
function normPoints(points, scale, thetaParam) {
...
function curveFromPoints(originalPoints, scale, theta) {
    let points = normPoints(originalPoints, scale, theta);
    let controlPoints = normPoints(originalPoints, scale*1.1, theta);
